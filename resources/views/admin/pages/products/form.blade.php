...
@section('breadcrumb')
    <div class="flex items-center gap-2 text-xs font-body text-gray-400">
        <a href="{{ route('admin.products.index') }}" class="hover:text-crimson-500 transition-colors">Products</a>
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
        <span class="text-gray-600">{{ $isEdit ? $product->name : 'New Product' }}</span>
    </div>
@endsection
@section('content')<form method="POST" action="{{ $action }}" id="productForm" novalidate>
        @csrf
        @if ($isEdit)
            @method('PUT')
        @endif
        <div class="flex items-center justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="submit"
                    class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-crimson-500 text-white
        ...
        <input type="hidden" name="is_active" id="isActiveInput"
            value="{{ old('is_active', $isEdit ? ($product->is_active ? '1' : '0') : '1') }}">
        <div class="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-5">
            <div class="space-y-5">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div>
                            <label for="name" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                Module Name <span class="text-crimson-500">*</span>
                            </label>
                            <input type="text" id="name" name="name"
                                value="{{ old('name', $isEdit ? $product->name : '') }}"
                                placeholder="e.g. Laboratory Information Management System (LIMS)" required autofocus
                                class="w-full px-4 py-3 rounded-xl border text-sm font-body text-gray-800
                                      placeholder-gray-400 transition-all
                                      focus:outline-none focus:ring-1
                                      @error('name')
                                          border-crimson-500 ring-crimson-500 bg-crimson-500/3
                                      @else
                                          border-gray-200 focus:border-crimson-500 focus:ring-crimson-500
                                      @enderror">
                            @error('name')
                            ...
                            <div class="flex items-center gap-0">
                                <span
                                    class="px-3 py-3 bg-gray-50 border border-r-0 border-gray-200
                                         rounded-l-xl text-xs font-mono text-gray-400 shrink-0">
                                    /products/
                                </span>
                                <input type="text" id="slug" name="slug"
                                    value="{{ old('slug', $isEdit ? $product->slug : '') }}" placeholder="laboratory-lims"
                                    class="flex-1 px-4 py-3 rounded-r-xl border text-sm font-mono text-gray-700
                                          placeholder-gray-400 transition-all
                                          focus:outline-none focus:ring-1
                                          @error('slug')
                                              border-crimson-500 ring-crimson-500 bg-crimson-500/3
                        ...
                        <div>
                            <label for="tagline" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                ...
                                <span class="font-normal text-gray-400 ml-1">— shown in italic on the product card</span>
                            </label>
                            <input type="text" id="tagline" name="tagline"
                                value="{{ old('tagline', $isEdit ? $product->tagline : '') }}"
                                placeholder="e.g. Barcode specimens, analyzer integration & web-based reports"
                                maxlength="160"
                                class="w-full px-4 py-3 rounded-xl border text-sm font-body text-gray-800
                            ...
                            <div class="flex items-center justify-between mt-1">
                                @error('tagline')
                                    <p class="text-xs font-body text-crimson-600 flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                                                clip-rule="evenodd" />
                                ...
                                @else
                                    <span></span>
                                @enderror
                                <span class="text-[10px] font-body text-gray-400" id="taglineCount">
                                    {{ strlen(old('tagline', $isEdit ? $product->tagline : '')) }}/160
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="icon" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                    Icon Emoji
                                </label>
                                <div class="flex items-center gap-2">
                                    <div id="iconPreview"
                                        class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200
                                            flex items-center justify-center text-xl shrink-0">
                                        {{ old('icon', $isEdit ? $product->icon : '📦') }}
                                    </div>
                                    <input type="text" id="icon" name="icon"
                                        value="{{ old('icon', $isEdit ? $product->icon : '') }}" placeholder="📦"
                                        maxlength="10"
                                ...
                                <p class="mt-1.5 text-[10px] font-body text-gray-400">
                                    Paste any emoji from your keyboard or emojipedia.org
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" id="sort_order" name="sort_order"
                                    value="{{ old('sort_order', $isEdit ? $product->sort_order : 0) }}" min="0"
                                    max="999"
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                        ...
                        <span class="ml-auto font-body text-xs text-gray-400">
                            Shown in the middle column on the public products page
                        </span>
                    </div>

                    <div class="p-6">
                        <textarea id="description" name="description" rows="8"
                            placeholder="Write a detailed 4–6 sentence description explaining what this module does, why it matters, and how it works in the hospital context…"
                            class="w-full px-4 py-3 rounded-xl border text-sm font-body text-gray-800
                                     placeholder-gray-400 resize-y leading-relaxed transition-all
                                     focus:outline-none focus:ring-1
                                     @error('description')
                                         border-crimson-500 ring-crimson-500 bg-crimson-500/3
                                     @else
                                         border-gray-200 focus:border-crimson-500 focus:ring-crimson-500
                                     @enderror">{{ old('description', $isEdit ? $product->description : '') }}</textarea>
                        <div class="flex items-center justify-between mt-2">
                            ...
                            @else
                                <span></span>
                            @enderror
                            <span class="text-[10px] font-body text-gray-400" id="descCount">
                                {{ strlen(old('description', $isEdit ? $product->description : '')) }} chars
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                        ...
                        <span class="ml-auto font-body text-xs text-gray-400">
                            Shown as bullet points on the product card
                        </span>
                    </div>
                    ...
                    <div class="p-6">
                        <div id="featuresContainer" class="space-y-2 mb-3">
                            @php
                                $features = old('features', $isEdit ? $product->features ?? [] : []);
                                if (empty($features)) {
                                    $features = [''];
                                }
                            @endphp
                            ...
                            @foreach ($features as $i => $feat)
                                <div class="feature-row flex items-center gap-2 group/row">
                                    <div
                                        class="w-5 h-5 rounded-md bg-crimson-500/10 border border-crimson-500/20
                                    ...
                                    <input type="text" name="features[]" value="{{ $feat->feature_text }}"
                                        placeholder="e.g. Barcode specimen identification & phlebotomy wand scan"
                                    ...
                                    <button type="button" onclick="removeFeature(this)"
                                        class="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center
                                               ...
                                               hover:bg-crimson-500/5 transition-all shrink-0
                        ...
                        <button type="button" onclick="addFeature()"
                            class="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed
                                   border-gray-300 text-sm font-display font-600 text-gray-500
                                   hover:border-crimson-500/40 hover:text-crimson-600 hover:bg-crimson-500/3
                        ...
                        <p class="mt-3 text-[10px] font-body text-gray-400">
                            Tip: aim for 6–10 features. Each should be a concrete capability, not a vague claim.
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            <label for="meta_title" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                Meta Title
                            </label>
                            <input type="text" id="meta_title" name="meta_title"
                                value="{{ old('meta_title', $isEdit ? $product->meta_title : '') }}"
                                placeholder="Leave blank to use module name" maxlength="70"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm
                        ...
                        <div>
                            <label for="meta_description" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                Meta Description
                            </label>
                            <textarea id="meta_description" name="meta_description" rows="3" placeholder="Leave blank to use tagline"
                                maxlength="165"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm
                                         font-body text-gray-800 placeholder-gray-400 resize-none transition-all
                                         focus:outline-none focus:border-crimson-500 focus:ring-1 focus:ring-crimson-500">{{ old('meta_description', $isEdit ? $product->meta_description : '') }}</textarea>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
                        <div class="h-4 w-0.5 bg-crimson-500 rounded-full"></div>
                        <h2 class="font-display font-700 text-gray-900 text-sm">Categorisation</h2>
                    </div>
                    <div class="p-5 space-y-4">


                        <div>
                            <label for="category" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                Category <span class="text-crimson-500">*</span>
                            </label>
                            <select id="category" name="category" required
                                class="w-full px-4 py-3 rounded-xl border text-sm font-body text-gray-800
                                       transition-all focus:outline-none focus:ring-1 cursor-pointer
                                       @error('category')
                                           border-crimson-500 ring-crimson-500 bg-crimson-500/3
                                       @else
                                           border-gray-200 focus:border-crimson-500 focus:ring-crimson-500
                                       @enderror">
                                <option value="" disabled @selected(!$isEdit)>Choose a category…</option>
                                @foreach (\App\Models\Product::CATEGORIES as $catKey => $catLabel)
                                    <option value="{{ $catKey }}" @selected(old('category', $isEdit ? $product->category : '') === $catKey)>
                                        {{ $catLabel }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="category_label" class="block font-body text-xs font-600 text-gray-700 mb-1.5">
                                Category Label
                                <span class="font-normal text-gray-400 ml-1">— auto-set from category</span>
                            </label>
                            <input type="text" id="category_label" name="category_label"
                                value="{{ old('category_label', $isEdit ? $product->category_label : '') }}"
                                placeholder="Auto-populated when you choose a category"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm
                                      font-body text-gray-600 placeholder-gray-400 bg-gray-50 transition-all
                                      focus:outline-none focus:border-crimson-500 focus:ring-1 focus:ring-crimson-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="tag" name="tag"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm
                                       font-body text-gray-800 transition-all focus:outline-none
                                       focus:border-crimson-500 focus:ring-1 focus:ring-crimson-500 cursor-pointer">
                                <option value="">No tag</option>
                                @foreach (['HIS-Related', 'Custom Dev', 'Support', 'Advisory'] as $tagOption)
                                    <option value="{{ $tagOption }}" @selected(old('tag', $isEdit ? $product->tag : '') === $tagOption)>
                                        {{ $tagOption }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5">
                        ...
                        <input type="text" id="href" name="href"
                            value="{{ old('href', $isEdit ? $product->href : '') }}"
                            placeholder="/products/laboratory-lims"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm
                                  font-mono text-gray-700 placeholder-gray-400 transition-all
                                  focus:outline-none focus:border-crimson-500 focus:ring-1 focus:ring-crimson-500">
                        <p class="mt-2 text-[10px] font-body text-gray-400 leading-relaxed">
                            Leave blank — it is auto-generated as <code
                                class="bg-gray-100 px-1 rounded">/products/{slug}</code>
                    ...
                    <div class="p-5">
                        ...
                        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                            <div class="flex items-center gap-2.5 mb-3">
                                <div id="previewIcon"
                                    class="w-9 h-9 rounded-xl bg-gray-50 border border-gray-200
                                        flex items-center justify-center text-xl leading-none shrink-0">
                                    {{ old('icon', $isEdit ? $product->icon : '📦') }}
                                </div>
                                <div>
                                    <div id="previewName"
                                        class="font-display font-700 text-gray-900 text-sm leading-tight">
                                        {{ old('name', $isEdit ? $product->name : 'Module Name') }}
                                    </div>
                                    <div id="previewCategory" class="text-[10px] text-gray-400 mt-0.5">
                                        {{ old('category_label', $isEdit ? $product->category_label : 'Category') }}
                                    </div>
                                </div>
                            </div>
                            <p id="previewTagline" class="font-body text-xs text-gray-500 italic mb-3 leading-relaxed">
                                "{{ old('tagline', $isEdit ? $product->tagline : 'Tagline will appear here') }}"
                            </p>
                                ...
                                @foreach (array_slice($features, 0, 3) as $f)
                                    @if ($f)
                                        <div class="flex items-center gap-1.5 text-[10px] text-gray-500">
                                            <svg class="w-2.5 h-2.5 text-crimson-500 shrink-0" fill="currentColor"
                                                viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                                    d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                                    clip-rule="evenodd" />
                                            </svg>
                                            {{ is_object($f) ? $f->feature_text : $f }}
                                        </div>
                                    @endif
                                @endforeach
                            </div>
        ...
        <div class="mt-6 bg-white border border-crimson-500/15 rounded-2xl overflow-hidden">
            ...
            <div class="p-5">
                <p class="font-body text-xs text-gray-500 mb-4 leading-relaxed">
                    ...
                    The public product page will return a 404 immediately.
                </p>
    ...
    <script>
        (function() {
            ...
            const categoryLabels = {
                ...
                'clinical': 'Clinical & Departmental',
            ...
            function updateActiveUI() {
                ...
                if (isActive) {
                    ...
                    label.textContent = 'This product is active and visible on the public site';
                    ...
                    label.textContent = 'This product is inactive and hidden from the public site';
            ...
            nameInput.addEventListener('input', () => {
                ...
                // Also sync href field
                const hrefInput = document.getElementById('href');
                if (!hrefInput.value || hrefInput.value.startsWith('/products/')) {
                    hrefInput.value = '/products/' + slugInput.value;
                }
            ...
            const catLabelInput = document.getElementById('category_label');

            catSelect.addEventListener('change', () => {
                const label = categoryLabels[catSelect.value] || '';
                catLabelInput.value = label;
                document.getElementById('previewCategory').textContent = label || 'Category';
            });


            function updatePreview() {
                document.getElementById('previewName').textContent =
                    nameInput.value.trim() || 'Module Name';
        ...
        `).join('');
            ...
            // Listen for changes in existing feature inputs
        ...
        `;
            ...
            document.getElementById('productForm').addEventListener('submit', function(e) {
                ...
                if (!name || !slug || !category) {
                    ...
                    // Scroll to top to show the error banner if it exists,
                    // or highlight the first empty required field
                    const firstEmpty = [nameInput, slugInput, catSelect].find(el => !el.value.trim());
                    if (firstEmpty) {
                        firstEmpty.focus();
