...
$action  = $isEdit
    ? route('admin.team.update', $member)
...
══════════════════════════════════════════════ --}}
<form method="POST" action="{{ $action }}" id="teamForm" novalidate enctype="multipart/form-data">
    @csrf
    ...
    {{-- Hidden visible field — toggled by JS --}}
    <input type="hidden" name="is_visible" id="isVisibleInput" value="{{ old('is_visible', $isEdit ? ($member->is_visible ? '1' : '0') : '1') }}">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name"
                               ...
                               placeholder="e.g. Dr. Muhammad Arif" required autofocus
                    ...
                    <div>
                        ...
                        <textarea id="bio" name="bio" rows="4"
                                  placeholder="Write a concise 3–4 sentence bio highlighting expertise, experience, and role at REDSOL…"
                                  required
                        ...
                        <div class="flex items-center justify-between mt-1">
                            ...
                            <span class="text-[10px] font-body text-gray-400" id="bioCount">{{ strlen(old('bio', $isEdit ? $member->bio : '')) }}/500</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-[10px] font-body text-gray-400">Lower numbers appear first on the About page</p>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div id="socialLinksContainer" class="space-y-2 mb-3">
                        @php
                            ...
                            if (empty($socialLinks)) $socialLinks = ['linkedin' => ''];
                        @endphp
                        @foreach($socialLinks as $platform => $url)
                            <div class="social-row flex items-center gap-2 group/row">
                                ...
                                <input type="url" name="social_urls[]" value="{{ $url }}" placeholder="https://linkedin.com/in/..." class="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 text-sm font-body text-gray-800 placeholder-gray-400 focus:outline-none focus:border-crimson-500 focus:ring-1 focus:ring-crimson-500 transition-all">
                    ...
                    <button type="button" onclick="addSocialRow()" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-gray-300 text-sm font-display font-600 text-gray-500 hover:border-crimson-500/40 hover:text-crimson-600 hover:bg-crimson-500/3 transition-all w-full justify-center">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                        Add social link
                    </button>
                    <p class="mt-3 text-[10px] font-body text-gray-400">Tip: Include only professional profiles. URLs must start with https://</p>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 items-start">
                        ...
                        <div>
                            ...
                            <label for="photo" class="block cursor-pointer group">
                                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-crimson-500/40 hover:bg-crimson-500/2 transition-all">
                                    <div class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-3 group-hover:bg-crimson-500/10 transition-colors">
                                        <svg class="w-5 h-5 text-gray-400 group-hover:text-crimson-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <p class="font-body text-[10px] text-gray-400 mt-1">Portrait JPG/PNG, max 2MB · 3:4 ratio recommended</p>
                                </div>
                        ...
                        <div>
                            ...
                            <div id="photoNewPreview" class="hidden mb-3">
                                <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 flex items-center justify-center aspect-[3/4] relative">
                                    <img id="photoNewImg" src="" alt="New photo" class="w-full h-full object-cover rounded-lg">
                                    <span class="absolute top-2 right-2 text-[10px] font-display font-700 px-2 py-1 rounded-lg bg-crimson-500 text-white">New</span>
                                </div>
                            </div>
                            ...
                            <div id="photoCurrentWrap" class="mb-3">
                                <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 flex items-center justify-center aspect-[3/4] relative">
                                    <img src="{{ $member->photoUrl() }}" alt="{{ $member->photo_alt ?? $member->name }}" class="w-full h-full object-cover rounded-lg" id="photoCurrentImg">
                                    <span class="absolute top-2 right-2 text-[10px] font-display font-700 px-2 py-1 rounded-lg bg-green-500 text-white">Current</span>
                                </div>
                                {{-- Remove existing photo --}}
                                <label class="flex items-center gap-2 mt-2 cursor-pointer">
                                    <input type="checkbox" name="remove_photo" value="1" class="w-3.5 h-3.5 rounded border-gray-300 text-crimson-500 focus:ring-crimson-500/30 cursor-pointer" onchange="document.getElementById('photoCurrentWrap').style.opacity = this.checked ? '0.4' : '1'">
                                    <span class="font-body text-xs text-gray-500">Remove current photo</span>
                                </label>
                            </div>
                            @else
                            {{-- Empty placeholder --}}
                            <div id="photoEmptyState" class="bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center aspect-[3/4]">
                                <svg class="w-8 h-8 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                <p class="font-body text-[10px] text-gray-400 text-center">No photo uploaded yet</p>
                            </div>
                            @endif
                            ...
                            <div class="mt-3 space-y-1.5">
                                @foreach([
                                    ...
                                    'Portrait orientation (3:4 ratio) works best',
                                    'Avoid busy backgrounds — solid or blurred works well',
                                ] as $tip)
                                <div class="flex items-start gap-2">
                                    <svg class="w-3 h-3 text-crimson-500 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-200">
                        ...
                        <button type="button" id="visibleSwitch" onclick="toggleVisible()" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none shrink-0">
                            <span id="visibleSwitchKnob" class="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200"></span>
                        </button>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-200">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_leadership" id="leadershipInput" value="1" @checked(old('is_leadership', $isEdit ? $member->is_leadership : false)) class="sr-only peer">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden">
                ...
                <div class="p-5">
                    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                        {{-- Photo placeholder --}}
                        <div id="previewPhotoBg" class="h-32 bg-gray-100 rounded-lg mb-3 flex items-center justify-center">
                            @if($isEdit && $member->photo)
                                <img src="{{ $member->photoUrl() }}" alt="{{ $member->name }}" class="w-full h-full object-cover rounded-lg">
                            @else
                                <span id="previewInitials" class="font-display font-800 text-gray-300 text-3xl">{{ $member->initials ?? 'MA' }}</span>
                            @endif
                        </div>
                        {{-- Name + Position --}}
                        <div id="previewName" class="font-display font-700 text-gray-900 text-base leading-tight mb-0.5">{{ old('name', $isEdit ? $member->name : 'Team Member Name') }}</div>
                        <div id="previewPosition" class="font-body text-xs text-crimson-500 font-600 mb-1">{{ old('position', $isEdit ? $member->position : 'Job Title') }}</div>
                        @if(old('department', $isEdit ? $member->department : ''))
                            <div id="previewDepartment" class="font-body text-[10px] text-gray-400 mb-2">{{ old('department', $isEdit ? $member->department : '') }}</div>
                        @endif
                        {{-- Bio preview --}}
                        <p id="previewBio" class="font-body text-xs text-gray-500 leading-relaxed line-clamp-3">{{ Str::limit(old('bio', $isEdit ? $member->bio : ''), 100) }}</p>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                ...
                <div class="p-5 space-y-3">
                    @foreach([
                        ...
                        ['label' => 'Initials', 'value' => $member->initials],
                        ['label' => 'Leadership', 'value' => $member->is_leadership ? 'Yes' : 'No'],
                        ['label' => 'Sort Order', 'value' => $member->sort_order],
                        ['label' => 'Created', 'value' => $member->created_at->format('M j, Y')],
                        ['label' => 'Last Updated', 'value' => $member->updated_at->diffForHumans()],
                    ] as $info)
...
<div class="mt-6 bg-white border border-crimson-500/15 rounded-2xl overflow-hidden">
    ...
    <div class="p-5">
        <p class="font-body text-xs text-gray-500 mb-4 leading-relaxed">
            Permanently delete <strong class="text-gray-700">{{ $member->name }}</strong> from the team. This removes them from the public About page immediately. This action cannot be undone.
        </p>
        <form method="POST" action="{{ route('admin.team.destroy', $member) }}" onsubmit="return confirm('Permanently delete {{ addslashes($member->name) }} from the team? This cannot be undone.')" class="inline">
            @csrf
            @method('DELETE')
...
function updatePreview() {
    ...
    document.getElementById('previewPosition').textContent = document.getElementById('position').value.trim() || 'Job Title';
    ...
    document.getElementById('previewBio').textContent = bio ? (bio.length > 100 ? bio.substring(0, 100) + '…' : bio) : 'Professional bio will appear here.';
}
...
window.previewPhoto = function(input) {
    ...
    reader.onload = function(e) {
        ...
        previewBg.innerHTML = `<img src="${e.target.result}" alt="Preview" class="w-full h-full object-cover rounded-lg">`;
    };
    reader.readAsDataURL(file);
};

window.removePhoto = function() {
    document.getElementById('photoPreview').innerHTML = `<span id="previewInitials" class="font-display font-800 text-gray-300 text-3xl">${document.getElementById('name').value.slice(0, 2).toUpperCase() || 'MA'}</span>`;
...
document.getElementById('teamForm').addEventListener('submit', function(e) {
    ...
    if (!name || !position || !bio) {
        e.preventDefault();
        const firstEmpty = [nameInput, document.getElementById('position'), bioInput].find(el => !el.value.trim());
        if (firstEmpty) {
            firstEmpty.focus();
