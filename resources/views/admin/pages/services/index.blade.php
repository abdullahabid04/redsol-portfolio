...
@section('content')<div class="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-3 mb-6">
        ...
        <div
            ...
            <div class="relative">
                ...
                <div class="font-body text-xs text-gray-400 mt-0.5">Total services</div>
            </div>
        </div>
        ...
        <form method="GET" action="{{ route('admin.services.index') }}"
            ...
            <select name="tag"
                ...
                @foreach(\App\Models\Service::TAGS as $serviceTag)
                    <option value="{{ $serviceTag }}" @selected(request('tag') === $serviceTag)>{{ $serviceTag }}</option>
                @endforeach
            </select>
            ...
            <a href="{{ route('admin.services.create') }}"
                class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-crimson-500 text-white text-sm font-display font-600 hover:bg-crimson-600 transition-all hover:shadow-lg hover:shadow-crimson-500/25 shrink-0">
    ...
    @if(session('error'))
        <div data-flash
            class="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 border border-crimson-500/20 text-crimson-700 font-body text-sm mb-4">
    ...
    @endif<div class="flex items-center justify-between mb-3 px-1">
        ...
        @if($services->isEmpty())
            ...
            <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                ...
                <h3 class="font-display font-700 text-gray-900 text-lg mb-2">No services found</h3>
                <p class="font-body text-sm text-gray-400 max-w-sm mb-6">
                    @if(request('search') || request('tag') || request('status'))
                        No services match your current filters. Try clearing them or adjusting your search.
                    @else
                ...
                @if(request('search') || request('tag') || request('status'))
                    <a href="{{ route('admin.services.index') }}"
                        class="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-display font-600 text-gray-600 hover:border-crimson-500/30 hover:text-crimson-600 transition-colors">Clear
                        filters</a>
                @else
                    <a href="{{ route('admin.services.create') }}"
                        class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-crimson-500 text-white text-sm font-display font-600 hover:bg-crimson-600 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4" />
                        </svg>
                        Add first service
                            ...
                            <tr class="group hover:bg-gray-50/60 transition-colors duration-150" data-id="{{ $service->id }}">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2">
                                        <button type="button"
                                            class="sort-handle cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-500 transition-colors opacity-0 group-hover:opacity-100"
                                        ...
                                        <span
                                            class="font-body text-xs text-gray-400 w-4 text-center">{{ $service->sort_order ?: '—' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-xl bg-gray-50 border border-gray-200 flex items-center justify-center text-lg leading-none shrink-0 group-hover:border-crimson-500/20 transition-colors">
                                            {{ $service->icon ?? '📦' }}
                                        </div>
                                        <div class="min-w-0">
                                            <div
                                                class="font-body text-sm font-600 text-gray-900 group-hover:text-crimson-600 transition-colors truncate max-w-[180px]">
                                                {{ $service->name }}
                                            </div>
                                            <div class="font-body text-[10px] text-gray-400 font-mono mt-0.5">
                                                /services/{{ $service->slug }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 hidden md:table-cell">
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="font-body text-xs text-gray-500">{{ $featureCount }}
                                            feature{{ $featureCount !== 1 ? 's' : '' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <form method="POST" action="{{ route('admin.services.toggle', $service->id) }}" class="inline">
                                        ...
                                        <button type="submit"
                                            ...
                                            class="inline-flex items-center gap-1.5 text-[10px] font-display font-700 px-2.5 py-1.5 rounded-lg border transition-all duration-200 {{ $service->is_active ? 'bg-green-50 text-green-700 border-green-200 hover:bg-red-50 hover:text-crimson-600 hover:border-crimson-500/20' : 'bg-gray-100 text-gray-500 border-gray-200 hover:bg-green-50 hover:text-green-700 hover:border-green-200' }} group/toggle">
                                            <span
                                                class="w-1.5 h-1.5 rounded-full {{ $service->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                            <span
                                                class="group-hover/toggle:hidden">{{ $service->is_active ? 'Active' : 'Inactive' }}</span>
                                            <span
                                                class="hidden group-hover/toggle:inline">{{ $service->is_active ? 'Deactivate?' : 'Activate?' }}</span>
                                        </button>
                                    </form>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <a href="{{ route('admin.services.edit', $service->id) }}" title="Edit service"
