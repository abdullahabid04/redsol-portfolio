...
@php
    ...
    $formAction = $isEdit ? route('admin.clients.update', $client) : route('admin.clients.store');
    $pageTitle = $isEdit ? 'Edit Client' : 'Add New Client';
@endphp
@section('title', $pageTitle)
@section('page-title', $pageTitle)
@section('breadcrumb')
    <a href="{{ route('admin.dashboard') }}" class="text-xs font-body text-gray-400 hover:text-crimson-500 transition-colors">
        Dashboard
    </a>
...
@endsection
@section('content')<form method="POST" action="{{ $formAction }}" enctype="multipart/form-data" id="clientForm"
        novalidate>
        @csrf
        @if ($isEdit)
            @method('PUT')
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5">
            ...
            'Gilgit-Baltistan' => 'Gilgit-Baltistan',
                        ...
                        {{-- field to add modules that will store as json array in database --}}
                        <div>
                            <label class="block font-body text-xs font-600 text-gray-500 tracking-widest uppercase mb-1.5">
                                Modules Deployed
                                <span class="text-gray-400 font-400 normal-case tracking-normal ml-1">(add all modules used
                                    by this client)</span>
                            </label>
                            ...
                            <div id="modulesContainer" class="space-y-2">
                                @php
                                    ...
                                    $existingModules =
                                        $isEdit && $client->modules
                                            ? (is_array($client->modules)
                                                ? $client->modules
                                                : json_decode($client->modules, true) ?? [])
                                            : [];
                                    $modulesToDisplay = !empty($oldModules) ? $oldModules : $existingModules;

                                    // Ensure at least one field is shown
                                    if (empty($modulesToDisplay)) {
                                        $modulesToDisplay = [''];
                                    }
                                @endphp

                                @foreach ($modulesToDisplay as $index => $module)
                                    <div class="module-field flex items-center gap-2">
                                        <input type="text" name="modules[]" value="{{ $module }}"
                                            placeholder="e.g. Patient Registration, Billing, Pharmacy"
                                            class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 font-body text-sm text-gray-900 placeholder-gray-400 bg-white
                           hover:border-gray-300 focus:border-crimson-500 focus:ring-2 focus:ring-crimson-500/10 focus:outline-none transition-all
                           {{ $errors->has('modules.' . $index) ? 'border-crimson-500 bg-red-50' : '' }}">
                                        @if ($index > 0 || count($modulesToDisplay) > 1)
                                            <button type="button" onclick="removeModuleField(this)"
                                                class="p-2.5 rounded-xl border border-gray-200 text-gray-400 hover:text-crimson-500 hover:border-crimson-500 transition-all">
                                    ...
                                    @error('modules.' . $index)
                                        <p class="mt-1 text-xs font-body text-crimson-600">{{ $message }}</p>
                                    @enderror
                                @endforeach
                            </div>

                            <button type="button" onclick="addModuleField()"
                                class="mt-3 flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed border-gray-300 text-sm font-body text-gray-600 hover:border-crimson-500 hover:text-crimson-500 transition-all">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 4v16m8-8H4" />
                                </svg>
                        ...
                        <div>
                            ...
                            <input type="number" id="year_deployed" name="year_deployed"
                                value="{{ old('year_deployed', $isEdit ? $client->year_deployed : '') }}" min="2000"
                        ...
                        <div>
                            <label for="notes"
                                class="block font-body text-xs font-600 text-gray-500 tracking-widest uppercase mb-1.5">
                                Internal Notes
                                <span class="text-gray-400 font-400 normal-case tracking-normal ml-1">(not shown on
                                    site)</span>
                            </label>
                            <textarea id="notes" name="notes" rows="3"
                                ...
                                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 font-body text-sm text-gray-700 placeholder-gray-400 bg-white resize-none
                                   hover:border-gray-300 focus:border-crimson-500 focus:ring-2 focus:ring-crimson-500/10 focus:outline-none transition-all
                                   {{ $errors->has('notes') ? 'border-crimson-500 bg-red-50' : '' }}">{{ old('notes', $isEdit ? $client->notes : '') }}</textarea>
                            @error('notes')
                                <p class="mt-1.5 text-xs font-body text-crimson-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 items-start">
                            ...
                            <div>
                                ...
                                <label for="logo" class="block cursor-pointer group">
                                    <div
                                        ...
                                        <div
                                            class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-3
                            ...
                            <div>
                                <p class="font-body text-xs font-600 text-gray-500 tracking-widest uppercase mb-2">Preview
                                </p>


                                <div id="logoNewPreview" class="hidden mb-3">
                                    <div
                                        class="bg-gray-50 border border-gray-200 rounded-xl p-6 flex items-center justify-center aspect-video relative">
                                        <img id="logoNewImg" src="" alt="New logo"
                                            class="max-h-20 max-w-full object-contain">
                                        <span
                                            class="absolute top-2 right-2 text-[10px] font-display font-700 px-2 py-1 rounded-lg bg-crimson-500 text-white">
                                            New
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    <div id="logoCurrentWrap" class="mb-3">
                                        <div
                                            ...
                                            <img src="{{ $client->logoUrl() }}"
                                                ...
                                                class="max-h-20 max-w-full object-contain" id="logoCurrentImg">
                                            <span
                                                class="absolute top-2 right-2 text-[10px] font-display font-700 px-2 py-1 rounded-lg bg-green-500 text-white">
                                                Current
                                            </span>
                                        </div>
                                ...
                                @else
                                    <div id="logoEmptyState"
                                        class="bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center justify-center aspect-video">
                                ...
                                <div class="mt-3 space-y-1.5">
                                    @foreach (['SVG or PNG with transparent background works best', 'Shown on the homepage ticker and clients page', 'Avoid logos with white backgrounds — they won\'t show on white'] as $tip)
                                ...
                                @foreach (\App\Models\Client::TYPES as $val => $label)
                                    ...
                                    <label class="relative cursor-pointer">
                                        <input type="radio" name="type" value="{{ $val }}"
                                            {{ $selected ? 'checked' : '' }} class="peer sr-only" required>
                                        <div
                                            class="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl border-2 text-center transition-all
                                            border-gray-200 peer-checked:border-crimson-500 peer-checked:bg-crimson-500/5
                                            hover:border-gray-300">
                                            <span
                                                class="inline-block text-[10px] font-display font-700 tracking-wide px-2 py-0.5 rounded border {{ $badgePreview }}">
                                                {{ $label }}
                                            </span>
                        ...
                        <label class="flex items-start gap-4 cursor-pointer group">
                            <div class="relative mt-0.5 shrink-0">
                                <input type="checkbox" name="is_active" value="1"
                                    ...
                                    class="peer sr-only" id="isActiveToggle">

                                <div
                                    class="w-10 h-5 rounded-full border-2 border-gray-200 bg-white transition-all
                                        peer-checked:bg-crimson-500 peer-checked:border-crimson-500">
                                </div>

                                <div
                                    class="absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-gray-300 shadow-sm transition-all
                                        peer-checked:translate-x-5 peer-checked:bg-white">
                                </div>
                            </div>
                            <div>
                                <div
                                    class="font-display font-700 text-gray-900 text-sm group-hover:text-crimson-600 transition-colors">
                                    Active
                                </div>
                        ...
                        <label class="flex items-start gap-4 cursor-pointer group">
                            <div class="relative mt-0.5 shrink-0">
                                <input type="checkbox" name="is_featured" value="1"
                                    ...
                                    class="peer sr-only" id="isFeaturedToggle">
                                <div
                                    class="w-10 h-5 rounded-full border-2 border-gray-200 bg-white transition-all
                                        peer-checked:bg-crimson-500 peer-checked:border-crimson-500">
                                </div>
                ...
                @if ($isEdit)
                    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                        ...
                        <div class="p-5 space-y-3">
                            @foreach ([['label' => 'Client ID', 'value' => '#' . $client->id], ['label' => 'Location', 'value' => $client->locationString() ?: '—'], ['label' => 'Type', 'value' => $client->typeLabel()], ['label' => 'Year Deployed', 'value' => $client->year_deployed ?? '—'], ['label' => 'Sort Order', 'value' => $client->sort_order], ['label' => 'Created', 'value' => $client->created_at->format('M j, Y')], ['label' => 'Last Updated', 'value' => $client->updated_at->diffForHumans()]] as $info)
...
@push('scripts')
    <script>
        ...
        function addModuleField() {
            ...
            newField.className = 'module-field flex items-center gap-2';
            newField.innerHTML = `
        <input
            type="text"
            name="modules[]"
            ...
            class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 font-body text-sm text-gray-900 placeholder-gray-400 bg-white
                   hover:border-gray-300 focus:border-crimson-500 focus:ring-2 focus:ring-crimson-500/10 focus:outline-none transition-all"
        >
        <button type="button" onclick="removeModuleField(this)" class="p-2.5 rounded-xl border border-gray-200 text-gray-400 hover:text-crimson-500 hover:border-crimson-500 transition-all">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    `;
        ...
        function removeModuleField(button) {
            const field = button.closest('.module-field');
            field.remove();
