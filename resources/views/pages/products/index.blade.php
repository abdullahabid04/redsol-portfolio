...
@section('content')
    <section class="relative pt-36 pb-20 bg-gray-900 overflow-hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-gray-900 to-transparent pointer-events-none">
        </div>
        <div
        ...
        <div class="relative max-w-7xl mx-auto px-6 lg:px-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <span class="text-crimson-500 text-xs font-display tracking-widest uppercase font-600">HIS Product
                    Suite</span>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
                <div>
                    ...
                    <p class="font-body text-gray-400 text-lg leading-relaxed max-w-lg">
                        Every module in the REDSOL HIS is built around a single unified database — clinical, diagnostic,
                        administrative, and imaging workflows all talking to each other in real time.
                    </p>
                </div>


                <div class="grid grid-cols-3 gap-px bg-white/5 rounded-2xl overflow-hidden">
                    @foreach ([[$moduleCount . '+', 'Integrated Modules'], [$hospitalCount . '+', 'Hospitals Live'], ['1', 'Unified Database']] as $s)
            ...
            <div class="mt-14 flex flex-wrap gap-2" id="catNav">
                @foreach (['All Modules', 'Administration', 'Patient Journey', 'Clinical', 'Diagnostics & Imaging', 'Operations'] as $ci => $cat)
                    <button onclick="filterCat(this, '{{ Str::slug($cat) }}')"
        ...
        <div class="flex gap-10 animate-[ticker_40s_linear_infinite] w-max">
            @php
                $specs = [
                    ...
                    'One Database',
                    'Voice Reporting',
                    'Lossless PACS Compression',
                    'Multi-printer Support',
                    'Barcode Integration',
                    'Web-based Reports',
                    'SNOMED Histopathology',
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            @php
                $sections = [
                    'administration' => [
                        ...
                        'desc' =>
                            'The operational and security foundation of the entire platform — access control, reporting, HR, and asset management.',
                    ],
                    'patient-journey' => [
                        'label' => 'Patient Journey',
                        'desc' =>
                            'Every touchpoint from first registration to final discharge — registration, appointments, queuing, welfare and admission.',
                    ],
                    'clinical' => [
                        'label' => 'Clinical & Departmental',
                        'desc' =>
                            'Department-level clinical modules covering consultation, emergency, wards, surgery, gynecology and specialist centres.',
                    ],
                    'diagnostics' => [
                        'label' => 'Diagnostics & Imaging',
                        'desc' =>
                            'Full lab, radiology and PACS suite — DICOM, analyzers, voice reporting, lossless compression and unlimited licences.',
                    ],
                    'operations' => [
                        'label' => 'Operations & Supply Chain',
                        'desc' =>
                            'Billing, pharmacy dispensing, and inventory management — the financial and supply backbone of the hospital.',
                    ],
                ];
                $currentCat = '';
            @endphp
            ...
            @foreach ($allModules as $mi => $module)
                @if ($currentCat !== $module['category'])
                    @php
                        $currentCat = $module['category'];
                        $sec = $sections[$currentCat];
                    @endphp
                    <div class="module-section mt-16 mb-8 reveal" data-cat="{{ $currentCat }}">
                        <div class="flex items-center gap-4">
                            <div class="h-px flex-1 bg-gray-100 max-w-[60px]"></div>
                            <span
                                class="text-crimson-500 text-xs font-display tracking-widest uppercase font-600">{{ $sec['label'] }}</span>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="lg:col-span-1">
                                <div class="flex items-start gap-4 mb-6">
                                    ...
                                    <div class="flex-1">
                                        <span
                                            class="inline-block text-[10px] font-display font-600 tracking-wider uppercase px-2.5 py-1 rounded-full {{ $c['badge_bg'] }} {{ $c['badge_text'] }} border border-current/20 mb-2">
                                            {{ $module['cat_label'] }}
                                        </span>
                                ...
                                <p
                                    class="{{ $module['category'] === 'administration' || $module['category'] === 'diagnostics' ? 'text-gray-400' : 'text-gray-500' }} font-body text-sm leading-relaxed mb-6 italic">
                                    "{{ $module['tagline'] }}"
                                </p>
                            ...
                            <div class="lg:col-span-1">
                                <p
                                    class="{{ $module['category'] === 'administration' || $module['category'] === 'diagnostics' ? 'text-gray-400' : 'text-gray-600' }} font-body text-sm leading-[1.85]">
                            ...
                            <div class="lg:col-span-1">
                                <div class="flex items-center gap-2 mb-4">
                                    ...
                                    <span
                                        class="{{ $module['category'] === 'administration' || $module['category'] === 'diagnostics' ? 'text-gray-400' : 'text-gray-400' }} text-xs font-display tracking-widest uppercase font-600">Key
                                        Features</span>
                                </div>
                                <ul class="space-y-2.5">
                                    @foreach ($module['features']->take(5) as $feat)
                                        <li class="flex items-start gap-2.5">
                                            <svg class="w-3.5 h-3.5 text-crimson-500 mt-0.5 shrink-0" fill="currentColor"
                                                viewBox="0 0 20 20">
                                            ...
                                            <span
                                                class="{{ $module['category'] === 'administration' || $module['category'] === 'diagnostics' ? 'text-gray-400' : 'text-gray-600' }} font-body text-xs leading-relaxed">{{ $feat->feature_text }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
    ...
    <section class="py-24 bg-gray-900 border-t border-gray-800">
        <div class="max-w-4xl mx-auto px-6 text-center reveal">
            <div
                ...
                <span class="text-crimson-400 text-xs font-display tracking-widest uppercase font-600">See it in
                    action</span>
            </div>
            ...
            <p class="font-body text-gray-400 text-lg max-w-xl mx-auto mb-10">
                Schedule a free walkthrough of any module — or all 25 at once. Our team will show you exactly how REDSOL
                maps to your specific workflows.
            </p>
            <div class="flex flex-wrap gap-4 justify-center">
                <a href="/contact"
                    class="group flex items-center gap-3 px-10 py-4 rounded-2xl bg-crimson-500 text-white font-display font-700 text-sm hover:bg-crimson-600 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-crimson-500/30">
                    Request a Demo
                ...
                <a href="/services"
                    class="group flex items-center gap-3 px-10 py-4 rounded-2xl border border-white/15 text-gray-300 font-display font-600 text-sm hover:border-crimson-500/40 hover:text-crimson-400 transition-all duration-300">
                    View All Services
                </a>
            </div>
        </div>
